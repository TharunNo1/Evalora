...
class _UploadScreenState extends State<UploadScreen> {
  ...
  Future<void> _pickFile(Function(PlatformFile) onPicked,
...
Future<MultipartFile> _buildMultipartFile(PlatformFile pickedFile, String fieldName) async {
  if (kIsWeb) {
    // Web: use bytes
  ...
  if (checklistFile != null) {
    formData.files.add(
      MapEntry("founderChecklist", await _buildMultipartFile(checklistFile as PlatformFile, "founderChecklist")),
    );
  }
  ...
  if (pitchDeckFile != null) {
    formData.files.add(
      MapEntry("pitchDeck", await _buildMultipartFile(pitchDeckFile as PlatformFile, "pitchDeck")),
    );
  }
  ...
  if (supportingDocs.isNotEmpty) {
      ...
      formData.files.add(
        MapEntry("otherDoc1", await _buildMultipartFile(supportingDocs[0], "otherDoc1")),
      );
    
    if (supportingDocs.length >= 2) {
      formData.files.add(
        MapEntry("otherDoc2", await _buildMultipartFile(supportingDocs[1], "otherDoc2")),
      );
    }
  }
            ...
            Form(
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    decoration:
                        const InputDecoration(labelText: "Founder Email ID"),
                  ...
                  ListTile(
                    ...
                    title: Text(checklistFile == null
                        ? "Upload Checklist (PDF)"
                        : checklistFile!.name),
                    trailing: IconButton(
                      icon: const Icon(Icons.upload),
                      onPressed: () =>
                          _pickFile((file) => checklistFile = file, allowedExtensions: ["pdf"]),
                    ),
                  ),
                  ListTile(
                    leading: const Icon(Icons.slideshow),
                    title: Text(pitchDeckFile == null
                  ...
                  ListTile(
                    ...
                    title: Text(supportingDocs.isEmpty
                        ? "Upload up to 2 Supporting Documents"
                        : "${supportingDocs.length} documents selected"),
                    trailing: IconButton(
